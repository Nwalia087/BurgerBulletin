...
const NewsItem = (props) => {
  let { title, description, imgUrl, url, dayNDate, source, author } = props;
  const parsedDate = new Date(dayNDate);

  return (
    <div className="position-relative">
      <span className="position-absolute bg-success z-2 rounded-pill px-3 text-light" style={{ top: "3%", left: "1%" }}>
        {source}
      </span>
